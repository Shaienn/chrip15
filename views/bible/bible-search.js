...
(function (App) {
    'use strict'
    var log = require('intel').getLogger('App.View.Bible.Search');
	...
	onShow: function () {
	    this.activate(App.Settings.BibleSettings.bible_xml);
	},
	onDestroy: function () {
	},
	/******************/
	...
	input: function (e) {
		...
		if (this.book_validate(found[1]) == false) {
		    log.warn("Book validation failed");
		...
		if (found[3]) {
			...
			log.warn("There is no " + verse_number + " verse");
			this.Bible.Valid = false;
			return;
		    }
		...
		log.warn("Verses validation failed");
		this.Bible.Valid = false;
		return;
	    }
