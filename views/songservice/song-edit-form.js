...
(function (App) {
    'use strict';
    var log = require('intel').getLogger('App.View.SongService.Songs.EditForm');
    var self;
	...
	slidePreviewHandler: function () {
	    log.verbose("slidePreviewHandler");
	...
	chordsPreviewHandler: function () {
	    log.verbose("chordsPreviewHandler");
	...
	addSongPartHandler: function () {
	    log.verbose("addSongPartHandler");
	...
	showSongpartDetails: function (songpart) {
		...
		if (self.chords_preview_mode == false) {
			...
			number: 0,
			background: App.Settings.SongserviceSettings.background,
			height: screen_bounds.height,
			width: screen_bounds.width,
			text: preparedText,
			font: App.Settings.SongserviceSettings.font_family
		    }));
	...
	saveBtnHandler: function () {
	    log.verbose("saveBtnHandler");

	    var song_name = $(this.ui.songMeta).find(".song-name-input input").val();
	    if (song_name == "") {
		log.info("Song name should contain something");
		return;
	    }
