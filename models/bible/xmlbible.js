...
(function (App) {
    'use strict';
    var log = require('intel').getLogger('App.Model.XMLBible');
    App.Model.XMLBible = Backbone.Model.extend({
	    ...
	    log.verbose("activate");
            ...
            fs.readFile(this.xmlpath, function (err, data) {
                ...
                parser.parseString(data, function (err, result) {
                    ...
                    if (err != null) {
                        log.error("Parse bible file: " + err);
                        that.valid = false;
                        d.reject(false);
                    }

                    log.info("Bible version: " + that.xmlpath);
        ...
        getBook: function (bid) {
            ...
            if (typeof (this.bible.BIBLE.BOOK[bid]) == "undefined") {
                log.error("There is no book with bid: " + bid);
                return null;
            }
            var book = {
                bid: bid,
...
//                chapters: this.bible.BIBLE.BOOK[bid].CHAPTER,
        ...
        getChapter: function (bid, cid) {
            ...
            if (typeof (this.bible.BIBLE.BOOK[bid].CHAPTER[cid]) == "undefined") {
                log.error("There is no book with bid: " + bid + " and chapter with cid: " + cid);
                return null;
            }
